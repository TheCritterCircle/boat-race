...
class Game{constructor(i){this.stage=new createjs.Stage(i),this.room=new Room(this),this.stage.addChild(this.room),this.hud=new HUD(this),this.stage.addChild(this.hud),this.shipInfo={},window.onkeydown=this.bindEvent("inputDown"),window.onkeyup=this.bindEvent("inputUp")}bindEvent(i){return this[i].bind(this)}getSize(){return{w:this.stage.canvas.width,h:this.stage.canvas.height}}getScreenBounds(){var i=this.room?this.room.globalToLocal(0,0):{x:0,y:0};return{x:i.x,y:i.y,w:this.stage.canvas.width,h:this.stage.canvas.height}}emit(...i){this.socket&&this.socket.emit(...i)}on(...i){this.socket&&this.socket.on(...i)}join(i){var e=io(i);this.socket=e,e.on("connect",(function(){console.log("Connected to server")}));var s=this.room;s.bind("joinShip"),s.bind("addShip"),s.bind("removeShip"),e.on("pending",s.bind("reset",!0)),s.bind("fire"),s.bind("raceStart"),s.bind("moveShip"),s.bind("moveCrosshair")}update(i){this.player&&this.room.update(),this.hud.update(),this.stage.update()}mouseClick(i){this.fire(i)}fire(i){this.player&&1==this.player.mode&&(this.room.fire({id:this.player.id,name:this.player.name,crosshair:this.player.getCrosshairPos()}),this.emit("fire",this.player.getCrosshairPos()))}mouseMove(i){if(this.player&&1==this.player.mode){var e=this.player.globalToLocal(i.stageX,i.stageY);this.player.setCrosshairPos(e.x,e.y),e=this.player.getCrosshairPos(),this.emit("moveCrosshair",e)}}inputDown(i){if(this.player&&0==this.player.mode){switch(console.log("INPUT: "+i.keyCode),i.keyCode){case 38:case 87:this.player.moving.up=1;break;case 37:case 65:this.player.moving.left=1;break;case 40:case 83:this.player.moving.down=1;break;case 39:case 68:this.player.moving.right=1}var{up:e,down:s,left:t,right:h}=this.player.moving;0==e&&0==s&&0==t&&0==h||this.emit("moveShip",{x:this.player.worldX,y:this.player.worldY,moving:this.player.moving})}}inputUp(i){if(this.player&&0==this.player.mode)switch(console.log("INPUT: "+i.keyCode),i.keyCode){case 38:case 87:this.player.moving.up=0,this.emit("moveShip",{x:this.player.x,y:this.player.y,moving:this.player.moving});break;case 37:case 65:this.player.moving.left=0,this.emit("moveShip",{x:this.player.x,y:this.player.y,moving:this.player.moving});break;case 40:case 83:this.player.moving.down=0,this.emit("moveShip",{x:this.player.x,y:this.player.y,moving:this.player.moving});break;case 39:case 68:this.player.moving.right=0,this.emit("moveShip",{x:this.player.x,y:this.player.y,moving:this.player.moving})}}}
